package com.timecat.module.timetable.timetable_custom;
...
public class CalenderDateBuildAdapter extends OnDateBuildAapter {
    ...
    protected View onBuildMonthLayout(LayoutInflater mInflate, int width, int height) {
        View first = mInflate.inflate(R.layout.timetable_item_dateview_calender_first, null, false);
        //月份设置
    ...
    protected View onBuildDayLayout(LayoutInflater mInflate, int pos, int width, int height) {
        View v = mInflate.inflate(R.layout.timetable_item_dateview_calender, null, false);
    ...
    protected void initDateBackground() {
        for (int i = 1; i < 8; i++) {
            if (layouts[i] != null){
                textViews[i].setBackgroundDrawable(context.getResources().getDrawable(R.drawable.timetable_border_dateview_normal));
    ...
    protected void activeDateBackground(int weekDay) {
        if (textViews.length > weekDay && textViews[weekDay] != null) {
            textViews[weekDay].setBackgroundDrawable(context.getResources().getDrawable(R.drawable.timetable_border_dateview_highligh));
