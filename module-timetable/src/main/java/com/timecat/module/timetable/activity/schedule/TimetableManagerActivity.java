...
public class TimetableManagerActivity extends Activity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.timetable_activity_timetable_manager);
		initView();
		initEvent();
	...
	private void addCourseItemView() {
		...
		for (int i = 0; i < courseList.size(); i++) {
			...
			final View view = LayoutInflater.from(this).inflate(R.layout.timetable_item_coursemanager_layout, null);
	...
	public void onClick(View arg0) {
		...
		} else if (i == R.id.id_manager_allcheck) {
			allCheck();

		} else if (i == R.id.id_manager_alldelete) {
			if (delete.size() == 0)
				Toasty.warning(this, "没有选中的课程", Toast.LENGTH_SHORT).show();
			else {
